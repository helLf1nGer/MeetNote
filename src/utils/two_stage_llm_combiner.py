...
class TwoStageLLMCombiner:
    ...
    def _resolve_problematic_chunks(self, problematic_chunks: List[Dict], semantic_flow_output: List[Dict]) -> List[Dict]:
        ...
        for chunk_group in split_chunks:
            ...
            logger.info("Input to Groq LLM:")
            logger.info(prompt)
            
            response = self._generate_response(prompt)
            logger.info("Output from Groq LLM:")
    ...
    def _tokenize_chunks(self, chunks: List[Dict]) -> List[Dict]:
        for chunk in chunks:
            chunk['tokens'] = count_tokens(chunk['prev_text'] + chunk['text'] + chunk['next_text'])
        return chunks
    ...
    def _generate_response(self, prompt: str) -> str:
        ...
        response = groq_api_call(messages, self.model, max_tokens=500)
        return response
...
def combine(transcription: List[Dict], diarization: List[Dict]) -> List[Dict]:
    ...
    groq_combiner = TwoStageLLMCombiner()
    segments = groq_combiner.combine(semantic_flow_output, diarization)
